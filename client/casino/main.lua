...
function setVideoWallTvChannel()
    SetTvChannelPlaylist(0, Casino_Config.VideoType, true)
...
AddEventHandler(Casino_Config.EnterEvent, function()
    inCasino = true

    startCasinoThreads()
end)

AddEventHandler(Casino_Config.ExitEvent, function()
    inCasino = false
end)

AddEventHandler(Casino_Config.BigWinEvent, function()
    if not inCasino then
        return
    end
